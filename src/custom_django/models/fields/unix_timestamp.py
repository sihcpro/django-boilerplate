# encoding: utf-8
# source: https://github.com/myyang/django-unixtimestampfield
...
class TimestampPatchMixin(object):
    ...
    MAX_TS, MIN_TS = (
        253402271999.999,
        -719162,
    )  # 9999/12/31 23:59:59, 1/1/1 00:00:00
    ...
    def _datetime_to_timestamp(self, v):
        ...
        if timezone.is_aware(v):
            return (
                v - timezone.datetime(1970, 1, 1, tzinfo=timezone.utc)
            ).total_seconds()
        else:
    ...
    def to_timestamp(self, value):
        ...
        """
        if isinstance(value, (six.integer_types, float, six.string_types, Decimal)):
            try:
                return float(value)
            except ValueError:
    ...
    def to_default_timezone_datetime(self, value):
        ...
        return timezone.localtime(
            self.to_utc_datetime(value), timezone.get_default_timezone()
        )
    ...
    def from_number(self, value):
        ...
        if value > self.MAX_TS or value < self.MIN_TS:
            raise exceptions.ValidationError(
                ...
                "%s ~ %s (1/1/1 00:00:00 ~ 9999/12/31 23:59:59)"
...
class OrdinalPatchMixin(TimestampPatchMixin):
    ...
    def from_number(self, value):
        ...
        if value > self.MAX_OD or value < 1:
            raise exceptions.ValidationError(
                "Value out of range, acceptable: 1 ~ %s (1/1/1 ~ 9999/12/31)"
                % self.MAX_OD,
                code="out_of_rnage",
            )
...
class OrdinalField(OrdinalPatchMixin, UnixTimeStampField):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.auto_now, self.auto_now_add, self.use_numeric = (
            auto_now,
            auto_now_add,
            use_numeric,
        )
